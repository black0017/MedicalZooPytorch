...
def main():
    ...
    print(model)
    criterion = DiceLoss(classes=args.classes, skip_background_class=False)
    ...
    trainer = train.Trainer(args, model, criterion, optimizer, train_data_loader=training_generator,
                            valid_data_loader=val_generator, lr_scheduler=None, val_volume=full_volume)
...
def get_arguments():
    ...
    parser.add_argument('--terminal_show_freq', default=500)
    ...
    parser.add_argument('--model', type=str, default='UNET3D',
                        choices=('VNET', 'VNET2', 'UNET3D', 'DENSENET1', 'DENSEVOXELNET', 'DENSENET2', 'DENSENET3',
                                 'HYPERDENSENET', 'RESNETMED3D'))
    ...
    args.save = '../saved_models/' + args.model + '_checkpoints/' + args.dataset_name + '/{}_'.format(
        utils.datestr())
    return args
