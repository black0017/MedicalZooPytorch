...
class _Upsampling(nn.Sequential):
    def __init__(self, input_features, out_features):
        ...
        self.add_module('transp_conv_1',
                        nn.ConvTranspose3d(input_features, self.tr_conv1_features, kernel_size=2, stride=1))
        self.add_module('transp_conv_2',
                        nn.ConvTranspose3d(self.tr_conv1_features, self.tr_conv2_features, kernel_size=2, stride=1))
...
class DenseVoxelNet(nn.Module):
    ...
    def __init__(self, in_channels=1, classes=3):
        ...
        self.conv_init = nn.Conv3d(in_channels, num_input_features, kernel_size=1, stride=2, padding=1, bias=False)
        ...
        self.conv_final = nn.Conv3d(self.up_out_features, classes, kernel_size=1, bias=False)
        self.transpose = nn.ConvTranspose3d(self.dense_1_out_features, self.up_out_features, kernel_size=2,
                                            padding=1, stride=1)
...
def test():
    a = torch.rand(1, 1, 4, 4, 4)
    model = DenseVoxelNet(in_channels=1, classes=3)
    b, c = model(a)
    print(b.shape)
    print(c.shape)
    summary(model, (1, 4, 4, 4))
